...
udp_port = 11111  # Should be the same as the port in the robot program
BUFFER_SIZE = 1024
sock.bind((udp_host, udp_port))  # Bind the socket to the host and port

url = 'https://192.168.159.72:8080/video' #Replace with your own video url and add /video to the link

# Initialize robot_speed with a default value
robot_speed = 40

# Initialize lower and upper bounds for orange color in HSV
lower_orange = np.array([196, 77, 172])
upper_orange = np.array([255, 51, 54])
...
def detect_orange_ball(frame):
    """Function to detect a orange ball in the frame."""
    ...
    if contours:
        # Get the largest contour (assumed to be the orange ball)
...
def show_live_camera_feed():
    """Function to display live camera feed, detect orange ball, and control the robot."""
    global robot_speed, lower_orange, upper_orange, trail_buffer  # Declare global variables
    ...
    # Create trackbars for adjusting robot_speed and orange color threshold
    ...
    cv2.createTrackbar('Hue', 'Ball Chasing', lower_orange[0], 255, lambda x: None)
    ...
    while True:
        ...
        lower_orange[0] = cv2.getTrackbarPos('Hue', 'Ball Chasing')

        # Detect orange ball in the frame
        ball_x, ball_y, max_contour = detect_orange_ball(frame)
        
        # Create a mask only around the detected orange ball
        ...
        if ball_x is not None and ball_y is not None:
            cv2.circle(frame, (ball_x, ball_y), 20, (0, 255, 0), 2)  # Draw a orange circle around the ball
    ...
    # Check if the ball is inside the rectangle
        ...
        sock.sendto(response_message.encode("utf-8"), ('192.168.245.171', 11111))  # Send the command to the robot
